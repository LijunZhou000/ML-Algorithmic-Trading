...
# FULL RETRAIN → más épocas y aprendizaje desde cero
EPOCHS = 20                           # Más épocas para full retrain
BATCH_SIZE = 64
SEQUENCE_LENGTH = 60
TRAIN_SPLIT = 0.8
LEARNING_RATE = 0.001                 # LR más alto para full retrain
...
# ==================== FULL RETRAIN (desde cero) ====================
def full_retrain_model(X_train, y_train, X_val, y_val):
    model = LSTMModel(input_size=5).to(DEVICE)   # ← Modelo completamente nuevo
    model.train()
    logger.info(f"Creando modelo desde cero → FULL RETRAIN en {DEVICE}")
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            for batch_x, batch_y in val_loader:
                ...
                loss = criterion(output, batch_y)
                val_loss += loss.item()
                correct += ((output > 0.5) == (batch_y > 0.5)).sum().item()
...
async def retrain_symbol(symbol: str):
    try:
        ...
        logger.info(f"{symbol} → {len(X)} secuencias generadas → FULL RETRAIN")

        # Rutas
        model_path = MODEL_DIR / f"lstm_{symbol.lower()}.pth"
        scaler_path = MODEL_DIR / f"lstm_{symbol.lower()}_scaler.joblib"

        # FULL RETRAIN (nuevo modelo desde cero)
        new_model = full_retrain_model(X_train, y_train, X_val, y_val)
        ...
        logger.info(f"✅ Modelo FULL RETRAIN guardado: {new_model_path}")
...
async def main():
    parser = argparse.ArgumentParser(description="FULL RETRAIN semanal de modelos LSTM/GRU (PyTorch)")
    parser.add_argument('--symbol', type=str, help='Solo un símbolo (GC, CL, ES, 6B). Si se omite → todos')
    args = parser.parse_args()

    symbols_to_train = [args.symbol.upper()] if args.symbol else SYMBOLS

    for symbol in symbols_to_train:
        logger.info(f"=== INICIANDO FULL RETRAIN {symbol} ===")
        await retrain_symbol(symbol)
        await asyncio.sleep(5)

    logger.info("🎉 FULL RETRAIN COMPLETO. Modelos y scalers listos para el bot.")
