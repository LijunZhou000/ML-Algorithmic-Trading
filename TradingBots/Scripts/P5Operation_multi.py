...
parser.add_argument('--symbol', type=str, required=False, help='Ticker único (ej: GC). Omitir para operar todos los futuros disponibles.')
...
PREDICCION_CADA_MINUTOS = 240        # Intervalo con posición abierta (4 horas)
PREDICCION_SIN_POSICION_MINUTOS = 15  # Intervalo sin posición (búsqueda de entrada)
PREDICCION_GRACE_MINUTOS = 30        # Margen para catch-up (± minutos sobre horario)
...
# Rutas estáticas/dinámicas de futuros
INFO_DIR = Path(__file__).resolve().parents[1] / 'Data' / 'Info'
FUTURES_STATIC  = INFO_DIR / 'base_info.json'
FUTURES_DYNAMIC = INFO_DIR / 'dynamic_info.json'

# Directorio donde se esperan los artefactos entrenados (.h5, .pkl, carpetas de modelos)
MODELS_DIR = Path(__file__).resolve().parents[1] / 'Models'
MODELS_DIR_ARTEFACTS = MODELS_DIR / 'models'
...
# Si no se pasa --symbol, lanzamos un proceso por cada futuro que tenga spec+modelo
if args.symbol is None:
    try:
        specs = load_specs()
    except Exception as e:
        logger.error(f"No se pudieron cargar specs: {e}")
        sys.exit(1)

    # Buscar artefactos de modelos. Preferimos la convención usada en notebooks:
    # MODELS_DIR / <asset.lower()> / trading_model_lstm_2level  (o _gru_)
    def find_model_artifact_for_ticker(ticker: str) -> Optional[Path]:
        t = ticker.lower()
        # 1) Ruta explícita usada por notebooks
        candidate_lstm = MODELS_DIR / t / 'trading_model_lstm_2level'
        candidate_gru  = MODELS_DIR / t / 'trading_model_gru_2level'
        if candidate_lstm.exists():
            return candidate_lstm
        if candidate_gru.exists():
            return candidate_gru

        # 2) Buscar recursivamente en MODELS_DIR_ARTEFACTS
        if MODELS_DIR_ARTEFACTS.exists():
            for p in MODELS_DIR_ARTEFACTS.rglob('*'):
                if p.is_dir() and t in p.name.lower():
                    if 'trading_model' in p.name.lower():
                        return p
                    try:
                        for child in p.iterdir():
                            if child.is_dir() and 'trading_model' in child.name.lower():
                                return child
                    except Exception:
                        continue

        # 3) Fallback: buscar recursivamente en MODELS_DIR
        if MODELS_DIR.exists():
            for p in MODELS_DIR.rglob('*'):
                if p.is_dir() and 'trading_model' in p.name.lower() and t in str(p).lower():
                    return p

        return None

    ticker_to_model = {}
    for ticker in specs.keys():
        model_path = find_model_artifact_for_ticker(ticker)
        if model_path is not None:
            ticker_to_model[ticker] = str(model_path)

    activos_disponibles = list(ticker_to_model.keys())

    if not activos_disponibles:
        logger.warning("No se encontraron futuros con spec y artefactos de modelos. Nada que lanzar.")
        sys.exit(0)

    script_path = Path(__file__).resolve()
    logger.info(f"Lanzando procesos para activos: {', '.join(sorted(activos_disponibles))}")
    for i, ticker in enumerate(sorted(activos_disponibles)):
        child_client_id = CLIENT_ID + i
        model_path_arg = ticker_to_model.get(ticker, MODEL_PATH)
        cmd = [sys.executable, str(script_path), '--symbol', ticker, '--exchange', EXCHANGE,
               '--currency', CURRENCY, '--client-id', str(child_client_id), '--model-path', model_path_arg]
        logger.info(f"Lanzando {ticker} -> clientId={child_client_id} model={model_path_arg}")
        subprocess.Popen(cmd)

    logger.info("Todos los procesos lanzados. Padre saliendo.")
    sys.exit(0)
...
async def main():
    ...
    try:
        while True:
            ...
            # Scheduler aligned to multiples of PREDICCION_CADA_MINUTOS desde 00:00 (catch-up)
            start_of_day = ahora.replace(hour=0, minute=0, second=0, microsecond=0)
            minutes_since_start = int((ahora - start_of_day).total_seconds() // 60)
            last_multiple = (minutes_since_start // PREDICCION_CADA_MINUTOS) * PREDICCION_CADA_MINUTOS
            last_scheduled = start_of_day + datetime.timedelta(minutes=last_multiple)

            ran_normal_interval = (
                last_prediction_time is None
                or ((ahora - last_prediction_time).total_seconds() / 60.0 >= intervalo_activo)
            )

            missed_and_within_grace = (
                (last_prediction_time is None or last_scheduled > last_prediction_time)
                and ((ahora - last_scheduled).total_seconds() / 60.0 <= PREDICCION_GRACE_MINUTOS)
            )

            if ran_normal_interval or missed_and_within_grace:
                if not can_trade:
                    logger.critical("🚨 RISK OFF → CERRANDO POSICIONES")
                    await close_all_positions(ib, contract)
                    last_prediction_time = last_scheduled if missed_and_within_grace else ahora
